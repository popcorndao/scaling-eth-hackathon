...
async function main() {
    ...
    const l1RpcProvider = new ethers.providers.JsonRpcProvider('https://kovan.infura.io/v3/9aa3d95b3bc440fa88ea12eaa4456161')
    ...
    const L1_ERC20 = await factory__L1_ERC20.connect(l1Wallet).attach("0x4f96fe3b7a6cf9725f59d353f723c1bdb64ca6aa")
    const L2_ERC20 = await factory__L2_ERC20.connect(l2Wallet).attach("0xda10009cbd5d07dd0cecc66161fc93d7c9000da1")
    ...
    const L1_CurveDepositZap = await factory__L1_MockCurveDepositZap.connect(l1Wallet).attach(
        "0x1e4b7bdb80e34b893ca7e7C8573a915AF77087C6"
    )

    console.log("L1_CurveDepositZap address: ", L1_CurveDepositZap.address)

    console.log("Deploying L1_MockYearnVault ...");
    const L1_YearnVault = await factory__L1_MockYearnVault.connect(l1Wallet).attach(
        "0x82A1da05A30274f636bfFE01E4Aea1D34351f59F"
    );

    console.log("L1_YearnVault address: ", L1_YearnVault.address)

    console.log("Deploying L1_Pool ...")
    const L1_Pool = await factory__L1_Pool.connect(l1Wallet).attach(
        "0x9E435dDd89C43322E799844C9e6217f543c4a51e"
    )
    ...
    const L2_Pool = await factory__L2_Pool.connect(l2Wallet).attach(
        "0x1027323eAE3d9E0260De307Ac53d7be1b1385520"
    )
    ...
    const approveTx = await L2_ERC20.connect(l2Wallet).approve(L2_Pool.address, 40000)
    ...
    const depositTx = await L2_Pool.connect(l2Wallet).deposit(2500, { gasLimit: 1250000 });
    ...
    const withdrawTx = await L2_Pool.connect(l2Wallet).requestWithdrawal(2500, { gasLimit: 1250000 });
    ...
    // // deposit oDAI into L1_Pool
    // console.log("Depositing oDAI into L1_Pool ... ");
    // const depositTx2 = await L2_Pool.connect(l2Wallet).deposit(200);
